...
let rec check_predicates predicates =
  ...
  | _,(Meta_parser.Pos pred)::predicates ->
    (Data.findlib_predicates_set_by_dune pred) &&
...
let rec get_plugin directory plugins requires entries =
  ...
  | (Meta_parser.Comment _)::entries -> get_plugin directory plugins requires entries
...
let find_plugin ~dir ~rest meta =
  ...
  in
  let directory = match directory with
    | None -> dir
    | Some pkg_dir ->
      if pkg_dir.[0] = '+' || pkg_dir.[0] = '^' then
        Filename.concat (Lazy.force Helpers.stdlib)
...
let lookup_and_load_one_dir ~dir ~pkg =
  ...
  if Sys.file_exists meta_file then
    ...
    let meta_file =
      Filename.concat dir (meta_fn ^ "." ^ pkg)
    in
    if Sys.file_exists meta_file then
      Some (load meta_file ~pkg)
    else
...
let lookup_and_summarize dirs name =
  ...
  let rec loop dirs =
    ...
    | [] -> (
            ...
            | Some meta ->
              find_plugin ~dir:(Lazy.force Helpers.stdlib) ~rest meta
          )
    | dir :: dirs -> (
...
let load_gen ~load_requires dirs name =
  ...
  if not (Hashtbl.mem loaded_libraries name) then begin

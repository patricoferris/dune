...
let sites_locations_plugins_code ~libs ~builtins =
  ...
  Variant.Set.iter Findlib.findlib_predicates_set_by_dune
    ~f:(fun variant -> pr buf "   | _, %S -> true" (Variant.to_string variant));
  pr buf "   | _, _ -> false";
  prlist buf "already_linked_libraries" (public_libs libs)
    ~f:(fun lib -> pr buf "%S" (Lib_name.to_string (Lib.name lib)));
  pr buf "open Sites_locations_plugins.Private_.Meta_parser";
  prlist buf "builtin_library"
    (Package.Name.Map.to_list builtins)
    ~f:(fun (name,meta) ->
      let meta = Meta.complexify meta in
      let meta =
        Meta.filter_variable
          ~f:(function "plugin" | "directory" | "requires" -> true | _ -> false)
          meta
      in
      pr buf "(%S,%s)"
      ...
      | Some special -> (
        ...
        | Sites_locations { data_module; plugins = false } ->
          let module_ =
            generate_and_compile_module cctx ~name:data_module ~lib
              ~code:
                (Build.return (sites_locations_code ()))
              ~requires:(Ok [ lib ])
              ~precompiled_cmi:true
          in
          process_libs libs
